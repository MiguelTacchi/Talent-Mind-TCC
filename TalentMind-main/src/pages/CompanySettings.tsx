...
export default function CompanySettings() {
  ...
  const handleChangePassword = () => {
    if (!currentPass || newPass.length < 6) {
      toast({
        title: "Erro",
        description: "Preencha corretamente. Senha mínima: 6 caracteres.",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="max-w-xl space-y-4">
        {[
          {
            ...
            value: user?.companyName || "Tech Solutions",
          },
          {
            label: "Email de Contato",
            value: user?.email || "contato@techsolutions.com",
          },
          { label: "CNPJ", value: "12.345.678/0001-00" },
        ].map((field) => (
          <div
            key={field.label}
        ...
        <Dialog open={passwordOpen} onOpenChange={setPasswordOpen}>
          <DialogTrigger asChild>
            <Button
              variant="outline"
              className="rounded-xl w-full h-12 justify-start gap-3"
            >
